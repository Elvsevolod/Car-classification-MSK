...
### Обязательный путь оценки

Ответ организаторов на вопрос 44 уточняет приоритет: обязательный минимум — batch-пайплайн, который одной командой получает `images/` и CSV и формирует `submission.csv`, `embeddings.npy`, `candidates.csv`. API, PostgreSQL и web-интерфейс являются дополнительной демонстрационной частью.

1. На Linux x86_64 или через buildx собрать и проверить `linux/amd64` образы приложения и `pgvector`, передать `docker save` архив с SHA-256 по [CONTEST_IMAGE_DELIVERY.md](CONTEST_IMAGE_DELIVERY.md).
2. На чистом стенде с заранее загруженными образами выполнить обязательный офлайн-пайплайн: `docker compose --profile inference run --rm --pull never inference`; валидатор должен подтвердить три файла. Команда не должна выполнять build или pull.
3. Проверить, что source review не находит query expansion: каждый query обрабатывается независимо, поиск использует только статичную gallery.
4. Сформировать финальные валидные артефакты сдачи и сохранить их вне Git.

### Дополнительная демонстрационная часть

5. Проверить полный сервис на заранее загруженных образах: `docker compose up -d --no-build --pull never`, offline Swagger UI и React-интерфейс.
6. Добавить browser/e2e smoke-test UI: выбор query или загрузка файла → BBox → Top-10; отдельно — сценарий отказа.
7. Подготовить презентацию и сценарий защиты: запуск → изображение/BBox → Top-10 → отказ → экспорт.

### Производительность

8. Подготовить отдельный CUDA/GPU-профиль и benchmark на RTX A5000 / CUDA 12.2. Согласно ответам организаторов: latency batch=1 — 300 прогонов после 50 прогревов с синхронизацией CUDA; throughput — batches 1/8/16/32 не менее 10 секунд, плюс VRAM, время загрузки весов и детерминизм. Текущий CPU runtime не следует заявлять как GPU-решение.
