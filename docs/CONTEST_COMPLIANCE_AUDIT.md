...
Обязательный экспортный pipeline, форматы файлов, контейнеризация, API, PostgreSQL + pgvector и offline Swagger UI реализованы и протестированы. Проект **не следует заявлять как полностью готовый по всем пунктам инженерной части**, пока не будут закрыты GPU-профиль с замерами и презентация.
