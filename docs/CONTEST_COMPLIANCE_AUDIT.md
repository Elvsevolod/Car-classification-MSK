...
| Dockerfile и `docker compose up` | Выполнено | multi-stage `Dockerfile`, `docker-compose.yml`, entrypoint с миграциями и healthcheck |
...
| Интерактивный OpenAPI/Swagger без сети | Не выполнено | `/openapi.json` локальный, но `/docs` загружает Swagger JS/CSS с CDN |
...
Обязательный экспортный pipeline, форматы файлов, контейнеризация, API и PostgreSQL + pgvector реализованы и протестированы. Проект **не следует заявлять как полностью готовый по всем пунктам инженерной части**, пока не будут закрыты offline Swagger UI, GPU-профиль с замерами и презентация.
